...
![Web3 Pi UPS — front three-quarter view](../img/ups-standalone.png){: .img-center style="max-width: 480px;"}
...
- **OLED display** — a small monochrome screen behind a smoked window, showing charge level, power mode, and input/output voltages. See [Display & Menu](display-menu.md) for every screen.
- **Two buttons** — the round red buttons flanking the display. Short presses cycle through the status screens; holding the **LEFT** button for 2 s on the Home screen opens the settings menu ([details](display-menu.md)).
- **USB-C OUT** — on the front face, labeled **OUT**. A single cable to the Pi carries both power and USB data.
- **Battery latch** — the red **PUSH** tab on the side edge. Press it and slide the battery off its rail; the battery is [hot-swappable](battery.md) while the UPS runs on external power.

## Rear

![Web3 Pi UPS — rear view](../img/ups-back.png){: .img-center style="max-width: 480px;"}
...
The enclosure houses two boards: the main power board and the display/button panel. An internal M.2 slot accepts the optional LTE-M module for out-of-band [connectivity](../connectivity/index.md) — no disassembly is needed for normal use.
