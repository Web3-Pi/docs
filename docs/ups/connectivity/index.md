...
The Web3 Pi UPS accepts an optional plug-in LTE-M module (Cat-M1 / NB-IoT cellular) that gives the UPS its own cellular link — completely independent of your home network and mains power. When the power goes out and your router goes dark, the UPS keeps reporting. The module ships with a pre-installed global IoT SIM and an included data plan — no SIM card, mobile contract, or carrier configuration needed (the SIM also carries the device's identity, so don't swap it).
...
- **Remote commands** — reboot or shut down the host, power-cycle the output, trigger a beep, and start/stop/restart the Ethereum clients running on the Pi.
- **Remote firmware updates** — both the modem module and the UPS controller can be [updated over the air](../firmware-update.md) from the web panel.
...
The module reports to one of three backends — exactly **one active at a time**. **MQTT is the default and recommended mode**: zero configuration, and the full [web panel](web-panel.md) experience. HTTP and Arkiv are alternatives for specific needs. All three authenticate every message; MQTT and Arkiv are always encrypted, and HTTP mode adds encryption when you use HTTPS.
...
| **Arkiv** | Web3 Pi's Arkiv-compatible blockchain — [Arkiv mode](arkiv-mode.md) | Tamper-evident records for special/compliance needs |
...
The choice persists across reboots and power loss. The module reboot only pauses telemetry for roughly half a minute to two minutes while it reconnects — power to the Pi and battery backup are handled in hardware and are never interrupted.
