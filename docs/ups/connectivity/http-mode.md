...
If you don't, a minimal VPS is set up in about five minutes and the smallest tier is more than enough — the reference server is a single zero-dependency Python script. For example, on [DigitalOcean](https://www.digitalocean.com){:target="_blank"}:

1. Sign up, then choose **Create → Droplets**.
2. Pick a region near you, the **Ubuntu 24.04 (LTS)** image, and the cheapest plan (from a few dollars a month, billed per second up to the monthly cap — destroy the droplet anytime and pay only for the time it existed).
3. Add your SSH key (or choose a root password) and click **Create Droplet**. After about a minute, note the droplet's **public IPv4 address** — that address goes into the device URL in step 3.
...
!!! warning "Firewall — the port must be open"
...
!!! tip "No hardware yet?"
    ...
    [examples directory](https://github.com/Web3-Pi/Web3-Pi-UPS/tree/main/examples/http-control-server){:target="_blank"}
