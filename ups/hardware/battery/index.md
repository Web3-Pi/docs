...
Runtime depends on your Raspberry Pi's workload; other NP-F-compatible packs also fit. Capacities above are for the recommended **Newell** packs — see [Specifications](https://docs.web3pi.io/ups/reference/specifications/#battery) for links.
...
The **Home** screen shows a battery icon (fill level = charge, scrolling bars while charging, blinking below 10 % on battery), the charge percentage, and a mode label. The **BATTERY** [detail screen](https://docs.web3pi.io/ups/hardware/display-menu/#detail-screens) adds battery voltage and charge current.
...
On battery, the buzzer beeps once every 30 s below 20 % charge and twice every 5 s below 10 % (unless the buzzer is muted via menu → **Sound** — see [Display & Menu](https://docs.web3pi.io/ups/hardware/display-menu/index.md)); the companion service can shut the Pi down safely before the battery runs out — see [Host Integration](https://docs.web3pi.io/ups/host-integration/index.md).

Note

The charge percentage is estimated from battery voltage and can read optimistically while charging.

See [Display & Menu](https://docs.web3pi.io/ups/hardware/display-menu/index.md) for the full screen reference and [Power](https://docs.web3pi.io/ups/power/index.md) for failover and charging behavior in detail.
