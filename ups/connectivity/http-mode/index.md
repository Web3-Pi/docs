...
1. **URL** sent in step 3 — right IP, right port, `http://` prefix, **no trailing slash**, and the **final URL only** — the UPS does not follow redirects (firmware esp32:0.8.10 or newer: a `3xx` is logged and nothing is applied; older firmware followed redirects instead).
